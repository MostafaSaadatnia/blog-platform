    ...
    withMethods((store, api: CommentsService = inject(CommentsService)) => {
        function load(slug: string) {
            ...
            api.list(slug).subscribe({
                ...
                error: (err: Error) => patchState(store, { loading: false, error: err?.message ?? 'Failed to load comments' }),
            });
        }
        ...
        function add(body: string) {
            ...
            api.add(slug, body).subscribe({
                ...
                error: (err: Error) => patchState(store, { posting: false, error: err?.message ?? 'Failed to post comment' }),
            });
        }
        ...
        function remove(id: number) {
            ...
            api.remove(slug, id).subscribe({
                ...
                error: (err: Error) => patchState(store, { deletingId: null, error: err?.message ?? 'Failed to delete comment' }),
            });
        }
